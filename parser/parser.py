...
class Parser():
    ...
    def printError(self, error: Errors, reference_token: TokenClass, context_token: TokenClass = None):
        ...
        if not self.silent:
            self.term.prRed("Parsing Error: ")
            match error:
                case Errors.DOUBLE_WHITESPACE:
                    self.term.print(f"Double whitespace found between two keywords on", file=sys.stderr, end="")
                    ...
                    self.term.prYellow("Tip: Language specification specifies only a single whitespace seperating each keywords (except string literals).\n")
                case Errors.UNTERM_STR:
                    self.term.print(f"Unterminated string literal on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Language specification prevents multi-line string.\n")
                case Errors.UNIDENT_KEYWORD:
                    self.term.print(f"Unidentified keyword '{reference_token.lexeme}' on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                case Errors.UNEXPECTED_CHAR_TLDR:
                    self.term.print(f"Unidentified character after TLDR on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Place commands in a newline after TLDR.\n")
                case Errors.UNTERM_MULTILINE_COMMENT:
                    self.term.print(f"Unterminated multiline comment on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.")
                    self.term.print(f" OBTW was found on", end="", file=sys.stderr)
                    self.term.prYellow(f"line {reference_token.error_context.line}.\n\n")
                    self.term.print(f"\t{reference_token.error_context.line} | {self.get_code_line(reference_token.error_context.line)}", file=sys.stderr)
                    self.term.print(f"\t.\n\t.\n\t.", file=sys.stderr)
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Multiline comments are ended by 'TLDR'.\n")
                case Errors.EXPECTED_HAI:
                    self.term.print(f"Expected 'HAI' but found '{reference_token.lexeme}' instead on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Lolcode programs starts with 'HAI' and ends with 'KTHXBYE'.\n")
                case Errors.EXPECTED_WAZZUP:
                    self.term.print(f"Expected 'WAZZUP' but found '{reference_token.lexeme}' instead on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Variable declaration section should always be after HAI.\n")
                case Errors.EXPECTED_BUHBYE:
                    self.term.print(f"Expected 'BUHBYE' but found '{reference_token.lexeme}' instead on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Variable declaration section must be closed with 'BUHBYE'.\n")
                case Errors.EXPECTED_IHASA:
                    self.term.print(f"Unexpected token '{reference_token.lexeme}' found on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                ...
                case Errors.EXPECTED_VARIDENT:
                    ...
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Declare variable using 'I HAS A <varident>'.\n")
                case Errors.UNEXPECTED_NEWLINE:
                    self.term.print(f"Unexpected newline at", file=sys.stderr, end="")
                    self.term.prYellow(f"line {context_token.line}.\n\n")
                    self.term.print(f"\tParsing line:", file=sys.stderr)
                    self.term.print(f"\t{context_token.line} | {self.get_code_line(context_token.line)}\n\n", file=sys.stderr)
                    self.term.print(f"\tNext token found on", end="", file=sys.stderr)
                    self.term.prYellow(f"line {reference_token.line}.\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n\n", file=sys.stderr)
                    self.term.prYellow("Tip: Lolcode commands are separated by a newline. Soft command breaks are not currently supported.\n")
                case Errors.UNEXPECTED_TOKEN:
                    self.term.print(f"Unexpected token '{reference_token.lexeme}' found on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                case Errors.INVALID_VAR_VALUE:
                    self.term.print(f"Invalid variable value '{reference_token.lexeme}' for '{context_token.lexeme}' on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Supported variable values are literals, variable identifier (reference), or expression.\n") 
                case Errors.UNEXPECTED_OPERAND:
                    self.term.print(f"Unexpected operand '{reference_token.lexeme}' for {context_token.classification} found on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Expressions in lolcode are in prefix notation.\n")
                case Errors.INVALID_STRING_CONT_ARG:
                    self.term.print(f"Unexpected argument '{reference_token.lexeme}' for {context_token.lexeme} found on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Only literals and variables are allowed in string concatenation.\n")
                case Errors.INVALID_ARG_SEPARATOR:
                    self.term.print(f"Unexpected token '{reference_token.lexeme}' for '{context_token.lexeme}' operation on", file=sys.stderr)
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Operations with multiple arities are separated by 'AN'.\n")
                case Errors.INCOMPLETE_EXPR:
                    self.term.print(f"Incomplete expression found for '{reference_token.lexeme}' operation on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                ...
                case Errors.NESTING_IF:
                    self.term.print(f"Nesting IF statements found on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Nesting IF statements are not yet supported.\n")
                case Errors.UNTERM_IF:
                    self.term.print(f"Unterminated IF-THEN statement found on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\tParsing line:", file=sys.stderr)
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n\n", file=sys.stderr)
                    self.term.print(f"\tIF-THEN statement clause:", file=sys.stderr)
                    self.term.print(f"\t{context_token.line} | {self.get_code_line(context_token.line)}\n\n", file=sys.stderr)
                    self.term.prYellow("Tip: IF-THEN statements are terminated by 'OIC'.\n")
                case Errors.NESTING_SC:
                    self.term.print(f"Nesting switch-case statements found on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Nesting switch-case statements are not yet supported.\n")
                case Errors.MISSING_CASE:
                    self.term.print(f"Unexpected token '{reference_token.lexeme}' found at line", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Switch-case requires at least one non-default case.\n")
                case Errors.INVALID_CASE_VAL:
                    self.term.print(f"Invalid value '{reference_token.lexeme}' for a case key found at line", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Literals are the only keys allowed for cases.\n")
                case Errors.UNTERM_SC:
                    self.term.print(f"Unterminated switch-case statement found on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\tParsing line:", file=sys.stderr)
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n\n", file=sys.stderr)
                    self.term.print(f"\tSwitch-case statement clause:", file=sys.stderr)
                    self.term.print(f"\t{context_token.line} | {self.get_code_line(context_token.line)}\n\n", file=sys.stderr)
                    self.term.prYellow("Tip: Switch-case statements are terminated by 'OIC'.\n")
                case Errors.EMPTY_CASE:
                    self.term.print(f"Empty case found on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Case fallthrough is not currently supported..\n")
                case Errors.NESTING_LP:
                    self.term.print(f"Nesting loop statements found on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Nesting loop statements are not yet supported.\n")
                case Errors.INVALID_LOOPIDENT:
                    self.term.print(f"Unexpected token '{reference_token.lexeme}' found at line", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Loop identifiers must follow the same conventions as a variable identifier.\n")
                case Errors.INVALID_STEP:
                    self.term.print(f"Invalid loop step '{reference_token.lexeme}' for loop {context_token.lexeme} found at line", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Loop step can be either UPPIN (increment by 1) or NERFIN (decrement by 1).\n")
                case Errors.INVALID_COUNTER:
                    self.term.print(f"Invalid loop counter '{reference_token.lexeme}' for loop {context_token.lexeme} found at line", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Variables are the only valid loop counters.\n")
                case Errors.INVALID_LOOP_COND:
                    self.term.print(f"Invalid loop condition '{reference_token.lexeme}' for loop {context_token.lexeme} found at line", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Loop conditions follow the format (TIL | WILE) <expr>.\n")
                case Errors.UNTERM_LOOP:
                    self.term.print(f"Unterminated loop statement found on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\tParsing line:", file=sys.stderr)
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n\n", file=sys.stderr)
                    self.term.print(f"\tLoop declaration:", file=sys.stderr)
                    self.term.print(f"\t{context_token.line} | {self.get_code_line(context_token.line)}\n\n", file=sys.stderr)
                    self.term.prYellow("Tip: Loop statements are terminated by 'IM OUTTA YR <loop_label>'.\n")
                case Errors.NESTING_MULT_ART:
                    self.term.print(f"Nesting multiple arity operations found at line", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Expression with infinite arities can not be nested.\n")
                case Errors.INVALID_FUNCIDENT:
                    self.term.print(f"Unexpected token '{reference_token.lexeme}' found at line", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Function identifiers must follow the same conventions as a variable identifier.\n")
                case Errors.INVALID_FUNCTION_PARAM:
                    self.term.print(f"Invalid function parameter '{reference_token.lexeme}' for function {context_token.lexeme} found at line", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Variable identifiers are the only allowed function parameters.\n")
                case Errors.UNTERM_FUNC:
                    self.term.print(f"Unterminated function declaration found on", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\tParsing line:", file=sys.stderr)
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n\n", file=sys.stderr)
                    self.term.print(f"\tFunction declaration:", file=sys.stderr)
                    self.term.print(f"\t{context_token.line} | {self.get_code_line(context_token.line)}\n\n", file=sys.stderr)
                    self.term.prYellow("Tip: Function declarations are terminated by 'IF U SAY SO'.\n")
                case Errors.ONLY_FUNC_DEC_ALLOWED:
                    self.term.print(f"Unexpected token '{reference_token.lexeme}' found at line", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.self.term.prYellow("Tip: Only function declarations are allowed before 'WAZZUP'.\n")
                case Errors.FUNCTION_OVERLOADING:
                    func = self.main_program.func_table.retrieve_func(reference_token.lexeme)
                    self.term.print(f"Function '{reference_token.lexeme}' from", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line} ")
                    self.term.print(f"is already defined at", file=sys.stderr, end="")
                    self.term.prYellow(f"line {func.how_iz_i.line} ")
                    self.term.print(f".\n\n", file=sys.stderr, end="")
                    self.term.print(f"\tRedefining function here:", file=sys.stderr)
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n\n", file=sys.stderr)
                    self.term.print(f"\tFunction already defined here:", file=sys.stderr)
                    self.term.print(f"\t{func.how_iz_i.line} | {self.get_code_line(func.how_iz_i.line)}\n\n", file=sys.stderr)
                    self.term.prYellow("Tip: Function overloading is not currently supported.\n")
                case Errors.NESTING_FUNC:
                    self.term.print(f"Nesting function declarations found at", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Functions can't be defined inside functins.\n")
                case Errors.INVALID_FUNC_DECLARATION:
                    self.term.print(f"Illegal function declaration found at", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Functions can only be declared between the 'HAI' and 'KTHXBYE' sections and must not be inside flow control statements.\n")
                ...
                case Errors.INVALID_RETVAL:
                    self.term.print(f"Invalid return value '{reference_token.lexeme}' found at", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Valid return values are expression, literal, and variables (must be in the parameter).\n")
                case Errors.INVALID_FUNCTION_CALL:
                    self.term.print(f"Invalid function name '{reference_token.lexeme}' in a function call statement found at", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Use the function identifier to call a function.\n")
                case Errors.INVALID_FUNCTION_ARG:
                    self.term.print(f"Invalid function argument '{reference_token.lexeme}' in a function call statement found at", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Function arguments can be a literal, variable, or an expression.\n")
                case Errors.VISIBLE_SEP_EXPECTED:
                    self.term.print(f"Unexpected '{reference_token.lexeme}' keyword in a visible statement found at", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: Visible arguments are separated by '+'.\n")
                case Errors.UNTERM_MULT_ARITY:
                    self.term.print(f"Unterminated '{reference_token.lexeme}' expression found at", file=sys.stderr, end="")
                    self.term.prYellow(f"line {reference_token.line}.\n\n")
                    self.term.print(f"\t{reference_token.line} | {self.get_code_line(reference_token.line)}\n", file=sys.stderr)
                    self.term.prYellow("Tip: 'ALL OF' and 'ANY OF' operations are terminated by 'MKAY'.\n")
